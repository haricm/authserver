...
server.grant(oauth2orize.grant.code(function (client, redirectURI, user, ares, done) {
    var code = utils.uid(config.token.authCodeLength);
...
server.grant(oauth2orize.grant.token(function (client, user, ares, done) {
    ...
    db.accessTokens.save(token, db.accessTokens.findExpirationDate(config.token.expiresIn), user.id, client.id, client.scope, function (err) {
        if (err) {
            return done(err);
        }
...
server.exchange(oauth2orize.exchange.code(function (client, code, redirectURI, done) {
    db.authorizationCodes.find(code, function (err, authCode) {
        ...
        db.authorizationCodes.delete(code, function (err, result) {
            ...
            var token = utils.uid(config.token.accessTokenLength);
            db.accessTokens.save(token, db.accessTokens.findExpirationDate(config.token.expiresIn), authCode.userID, authCode.clientID, authCode.scope, function (err) {
                if (err) {
                    return done(err);
                }
...
server.exchange(oauth2orize.exchange.password(function(client, username, password, scope, done) {
	...
	db.user.findOne( {username : username}, function (err, user) { 
        ...
        var token = utils.uid(config.token.accessTokenLength);
        db.accessTokens.save(token, db.accessTokens.findExpirationDate(config.token.expiresIn), user._id.toString(), client.id, scope, function (err) {
            if (err) {
                return done(err);
            }
            var refreshToken = null;
            ...
            if (scope && scope.indexOf("offline_access") === 0) {
                refreshToken = utils.uid(config.token.refreshTokenLength);
                db.refreshTokens.save(refreshToken, user._id.toString(), client.id, scope, function (err) {
                    if (err) {
                        return done(err);
                    }
                    return done(null, token, refreshToken, {expires_in: config.token.expiresIn});
...
server.exchange(oauth2orize.exchange.clientCredentials(function(client, scope, done) {
    ...
    db.accessTokens.save(token, db.accessTokens.findExpirationDate(config.token.expiresIn), null, client.id, scope, function (err) {
        if (err) {
            return done(err);
        }
    ...
    function (req, res, next) {
        ...
        db.clients.findByClientId(req.query.client_id, function(err, client) {
            if(!err && client && client.trustedClient && client.trustedClient === true) {
                ...
                res.render('dialog', { title : config.site.title, transactionID: req.oauth2.transactionID, user: req.user, client: req.oauth2.client });
            }
        });
    }
];
